import { z } from 'zod';
import { clienteDocumentoSchema, detalleItemSchema, cuotaSchema, medioPagoSchema, detraccionSchema, percepcionSchema } from './common.schema';
...
export const invoiceSchema = z.object({
  ...
  percepcion: percepcionSchema.optional(),
  medios_pago: z.array(medioPagoSchema).optional(),
  observaciones: z.string().max(1000).optional().or(z.literal('')),
  numero_orden_compra: z.string().max(50).optional().or(z.literal('')),
}).superRefine((data, ctx) => {
  // Credito requires cuotas
  ...
  if (data.fecha_vencimiento && data.fecha_emision && data.fecha_vencimiento < data.fecha_emision) {
    ctx.addIssue({
      code: z.ZodIssueCode.custom,
