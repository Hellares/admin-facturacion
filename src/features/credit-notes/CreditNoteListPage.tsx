...
export default function CreditNoteListPage() {
  ...
  const columns: ColumnsType<CreditNote> = [
    ...
    { title: 'Total', key: 'total', width: 120, align: 'right', render: (_: unknown, r: CreditNote) => <MoneyDisplay amount={r.totales?.total ?? 0} moneda={r.moneda as Moneda} strong fontSize={12} /> },
    { title: 'Origen', dataIndex: 'origen', width: 70, responsive: ['md'], render: (origen: 'web' | 'api' | undefined) => <OrigenTag origen={origen} /> },
    { title: 'Estado', dataIndex: 'estado_sunat', width: 110, render: (s: SunatStatus, record: CreditNote) => <SunatStatusBadge status={s} anulado={record.anulado} sunatInfo={record.sunat ?? record.respuesta_sunat} /> },
    {
      title: 'Acciones',
      key: 'actions',
      width: 220,
